...
<div id="{{ modal_id }}" class="js-modal {% if modal_mobile_full_screen %}js-fullscreen-modal{% endif %} {% if desktop_overlay_only %}js-modal-overlay-md{% endif %} modal modal-{{ modal_class }} modal-{{modal_position}} modal-{{modal_position_desktop}}-md transition-{{modal_transition}} modal-{{modal_width}} transition-soft {% if modal_zindex_top %}modal-zindex-top{% endif %} {% if settings.show_tab_nav %}modal-with-tabnav{% endif %}" style="display: none;" {% if data_component %}data-component="{{ data_component }}"{% endif %} {% if custom_data_attribute %}data-{{ custom_data_attribute }}="{{ custom_data_attribute_value }}"{% endif %}>
    ...
    {% endif %}
                <div class="js-modal-close {% if modal_mobile_full_screen %}js-fullscreen-modal-close{% endif %} {% if modal_zindex_top %}js-close-over-modal{% endif %} modal-header {% if not modal_header_title or modal_header_align == 'left' %}text-left{% endif %} {{ modal_header_class }}">
                    {% if modal_header_title %}
                        <div class="row align-items-center">
                            <div class="col{% if modal_header_align != 'left' %} offset-2 text-center{% endif %}">
                                {% block modal_head %}{% endblock %}
                            </div>
                            <div class="{% if modal_header_align == 'left' %}col-auto{% else %}col-2{% endif %} text-right">
