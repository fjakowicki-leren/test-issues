...
{% elseif use_account %}
	<span class="utilities-container">
		<a href="{% if not customer %}{{ store.customer_login_url }}{% else %}{{ store.customer_home_url }}{% endif %}" {% if tabnav %}class="tabnav-link"{% endif %}>
			<img src="{{ 'images/user-dog.png' | static_url }}" class="icon-inline utilities-icon utilities-icon-user {{ tabnav_icon_class }}" alt="{{ 'Mi cuenta' | translate }}" width="35" height="35">
		</a>
		{% if settings.utilities_type_desktop == 'icons_text' %}
